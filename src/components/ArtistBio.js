...
const ArtistBio = ({ bio, setArtistBio, artistName }) => {
  useEffect(() => {
    ...
    if (pageId !== "-1" && pages[pageId].extract && keywords.some((keyword) => (pages[pageId].extract.match(new RegExp(keyword, "gi")) || []).length >= 1)) {
    ...
    if (retryPageId !== "-1" && retryPages[retryPageId].extract && keywords.some((keyword) => (retryPages[retryPageId].extract.match(new RegExp(keyword, "gi")) || []).length >= 2)) {
